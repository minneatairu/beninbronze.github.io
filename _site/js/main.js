...
function renderSearchResults(results) {
  ...
  if (results && results.length > 0) {
    results.forEach((item, index) => {
      ...
      // Add mouse event listeners to show/hide image popup
      resultItem.querySelector('img').addEventListener('mouseenter', () => showImagePopup(item, index));
      resultItem.querySelector('img').addEventListener('mouseleave', hideImagePopup);

      searchResultsContainer.appendChild(resultItem);
    });
  } else {
    searchResultsContainer.innerHTML = 'No Records found';
  }
}
